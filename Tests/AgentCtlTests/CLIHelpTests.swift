  ...
  // Nested in the serialized `AgentCtlSuite` because it installs a runtime, which is process-global: so do the
  // CLI command tests, and the two must never overlap.
